				        ...
				        <span class="dropdown">
				            ...
				            <div class="dropdown-menu" role="menu" style="z-index: 1050; position: absolute; right: 50%; transform: translateX(50%);">
				                <% if (role != null && role == 2 && UserId == clientId) { %>
				                    <a class="dropdown-item" role="presentation" onclick="openEditJobModal('<%= job.getJobId() %>', '<%= job.getTitle() %>', '<%= job.getCompanyName() %>', '<%= job.getDescription() %>', '<%= job.getLocation() %>', '<%= job.getDeadline() %>', '<%= job.getSalary() %>')">Chỉnh sửa</a>
				                <% } %>
				                <% if (role != null && (role == 2 && UserId == clientId || role == 1)) { %>
				                    <a class="dropdown-item" role="presentation" onclick="deleteJob('<%= job.getJobId() %>')">Xóa</a>
				                <% } %>
				            </div>
				        </span>
				    </div>
